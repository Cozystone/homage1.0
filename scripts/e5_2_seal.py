...
SEAL_DIR = Path("data/e5_transfer_seal_2")
...
GATE = {"rise_required_relative": 0.05, "both_arms": True, "pursued_must_match": True}
...
COMMANDS = {
    ...
    "B2": ("python scripts/run_acquisition_daemon.py --local --table --no-curiosity "
           ...
           "--min-pressure 2 --state data/acquisition_daemon/e5_2_b2"),
}
...
def seal() -> None:
    ...
    doc = {
        "prereg": "docs/ATANOR_E5_2_transfer_prereg.md",
    ...
    print(f"  gate           : both arms +{GATE['rise_required_relative']:.0%}, pursued must match")
...
def score() -> None:
    ...
    need = gate["rise_required_relative"]
    ...
    if missing:
        ...
        worst = min(b1["relative_change"], b2["relative_change"])
        if worst >= need:
            verdict, why = "PASS", (f"both arms rose: B1 {b1['relative_change']:+.1%}, "
                                    f"B2 {b2['relative_change']:+.1%} (weakest {worst:+.1%})")
        else:
            weak = "B1" if b1["relative_change"] < b2["relative_change"] else "B2"
            verdict, why = "FAIL", (f"{weak} did not clear +{need:.0%}: B1 "
                                    f"{b1['relative_change']:+.1%}, B2 {b2['relative_change']:+.1%}")
