...
// Phone-link pairing passthrough to the LOCAL engine. The desktop (icons, orb) talks
// to /api/phone-link/* on :3000; the lane lives in the engine on 127.0.0.1:8502.
...
async function forward(req: NextRequest, path: string[], method: "GET" | "POST") {
  ...
  for (const base of backendBaseCandidates()) {
    try {
      const res = await fetch(`${base}/api/phone-link/${suffix}`, {
        method,
